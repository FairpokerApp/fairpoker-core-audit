...
describe('ChatRoom', () => {
  ...
  testHostAndGuest('announcing client versions', async ({hostChatRoom, guestChatRoom}) => {
    const claim = {
      appVersion: '0.1.0',
      sourceFingerprint: 'sha256:test',
      runtimeKind: 'ipfs',
      runtimeLabel: 'IPFS 固定入口',
      runtimeCid: 'QmRxJQFpbB4g5jk6ahSxUrqBbogYjVd5eJZb72ym1PgSTq',
    };
    const versionReceivedByHostPromise = listenOnce(hostChatRoom, 'clientVersion');
    const versionReceivedByGuestPromise = listenOnce(guestChatRoom, 'clientVersion');

    await hostChatRoom.announceClientVersion(claim);

    expect(await versionReceivedByHostPromise).toEqual([claim, 'hostid']);
    expect(await versionReceivedByGuestPromise).toEqual([claim, 'hostid']);
  });
});
